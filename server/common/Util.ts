    ...
    for (const controller of storage.controllers) {
        ...
        for (const action of actions) {
            ...
            const finalParams: {
                ...
                hasParams: boolean,
                hasQueries: boolean,
            } = { body: [], bodyClass: null, params: [], paramsClass: null, queries: [], queriesClass: null, hasBody: false, hasParams: false, hasQueries: false }
            for (const param of paramsToBeIncluded) {
                ...
                switch (param.type) {
                    case "body-param":
                        ...
                        // finalParams.body.push({ [param.name!]: { type: explicitType!, import: !primitiveTypes.includes(explicitType!) } })
                        break;
                    case "body":
                        finalParams.hasBody = true
                        if (!primitiveTypes.includes(explicitType!)) imports.add(explicitType!)
